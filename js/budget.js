...
function printVoucher(r, items, total) {
  ...
  // صفوف فارغة إضافية لتعبئة يدوية لو احتجت تطبع نسخة وتضيف فواتير بخط اليد لاحقًا
  const emptyRows = Array.from({ length: Math.max(0, 3 - items.length) }).map((_, i) => `
    <tr><td>${items.length + i + 1}</td><td></td><td></td><td></td><td class="desc"></td><td class="amt"></td></tr>`).join('');
...
<style>
  * { box-sizing: border-box; }
  ...
  .sign { display:flex; justify-content:space-between; margin-top:50px; gap:14px; }
  .sign div { flex:1; text-align:center; font-size:12.5px; }
  .sign .box { margin-top:8px; border:1px solid #999; border-radius:6px; height:56px; display:flex; align-items:flex-end; justify-content:center; padding-bottom:6px; color:#16233A; font-weight:600; }
